    ...
    */
    ...
    public function addWriteConnection($name = 'default') {
        if (isset(self::$configs['write'][$name])) {
            // 标准化为可用于负载均衡过程选取访问节点的格式.
            if (!isset(self::$fmtConfigs['write'][$name])){
                $_cfg = self::$configs['write'][$name];
                if (!isset( $_cfg['dsn'])){
                    if (isset($_cfg['host'])){
                        $_cfg['dsn'] = "mysql:host={$_cfg['host']};port={$_cfg['port']};dbname={$_cfg['db']}";
                        self::$fmtConfigs['write'][$name] =  $this->parseCfg($_cfg);
                    } else {
                        self::$fmtConfigs['write'][$name] = $_cfg;
                    }
                }
            }
            $cfg = self::$fmtConfigs['write'][$name];
